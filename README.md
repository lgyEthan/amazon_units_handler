...
Units, Pairs, Dozen 중 원하는 입력 단위와 Hybrid Box 또는 Large box 기준을 선택하면 필요한 box 수량과 단위별 환산 결과를 바로 계산할 수 있습니다.

## 사용 방법

1. 위 링크를 엽니다.
2. `Units`, `Pairs`, `Dozen` 중 입력 단위를 선택합니다.
3. `Hybrid Box (12 units)` 또는 `Large box (18 units)` 기준을 선택합니다.
...
- Large box: 18 units = 108 pairs = 9 dozen = 1 box
